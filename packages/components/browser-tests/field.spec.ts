...
test.describe('ADS field composition', () => {
  ...
  test('registers field, label, description, and error primitives', async ({ page }) => {
    expect(await page.evaluate(() => ['ads-field', 'ads-label', 'ads-description', 'ads-error'].every((tag) => Boolean(customElements.get(tag))))).toBe(true);
  });
  ...
  test('labels and focuses a slotted native control and exposes supporting text', async ({ page }) => {
    await page.locator('#sandbox').evaluate((sandbox) => {
      sandbox.innerHTML = `<ads-field label="Email address" required>
    ...
    await expect(input).toHaveAttribute('aria-description', 'Used for receipts. Enter a valid email.');
  ...
  test('propagates field context into an ADS shadow-backed input without duplicating form logic', async ({ page }) => {
    await page.locator('#sandbox').evaluate((sandbox) => {
      sandbox.innerHTML = `<ads-field label="Search query">
    ...
    // Both field and input expose label-text. Verify that this real click targets the outer field label.
    const outerLabel = page.locator('ads-field').locator('[part="label-text"]').first();
    expect(await outerLabel.evaluate((node) => (node.getRootNode() as ShadowRoot).host.localName)).toBe('ads-field');
    await outerLabel.click();
    await expect(input).toBeFocused();
  });

  test('restores a consumer-owned aria-description when field description is removed', async ({ page }) => {
    await page.locator('#sandbox').evaluate((sandbox) => {
      sandbox.innerHTML = `<ads-field id="field" label="Name">
    ...
    await page.locator('#help').evaluate((node) => node.remove());
  ...
  test('standalone label/error primitives expose visual required and alert semantics', async ({ page }) => {
    await page.locator('#sandbox').evaluate((sandbox) => {
      sandbox.innerHTML = '<ads-label required>Username</ads-label><ads-error>Username is required.</ads-error>';
    });
    await expect(page.locator('ads-label').locator('[part="required"]')).toHaveAttribute('aria-hidden', 'true');
    // textContent of the shadow span excludes assigned slot nodes. Assert the actual accessible tree.
    await expect(page.getByRole('alert')).toMatchAriaSnapshot('- alert: Username is required.');
  });
});
